...
st.markdown("""
    ...
    """, unsafe_allow_html=True) # FIXED: Changed from unsafe_content_type to unsafe_allow_html
            ...
            with st.spinner("Auditing signals and mirroring language..."):
                try:
                    ...
                    if output and "DRAFT:" in output:
                        ...
                        st.error("AI Output error. Please try clicking the button again.")
                except Exception as e:
...
if 'draft' in st.session_state:
    ...
    with col_left:
        ...
        st.caption("Instructions: Address the red [BRACKETED ITEMS] by typing your data directly here.")
        # User edits the draft here
    ...
    with col_right:
        ...
        if resolved_all:
            st.balloons()
            st.success("Audit Complete! Signals verified.")
            ...
            st.info("The download button will unlock once you address all [METRIC_NEEDED] and [KEYWORD_MISSING] items.")

else:
    st.info("Upload your resume and the job description in the sidebar to start.")
